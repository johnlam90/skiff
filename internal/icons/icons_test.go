...
// for the bulk of files in any project. The expected glyphs are spelled
// out as literal codepoints rather than read back out of extIcons: an
// expectation sourced from the map under test is a tautology that stays
// green even after two entries are swapped.
func TestForByExtension(t *testing.T) {
	cases := []struct {
		...
		{"main.go", "\ue627"},         //  nf-seti-go
		{"app.py", "\ue606"},          //  nf-seti-python
		{"index.JS", "\ue74e"},        //  nf-dev-javascript (case-insensitive)
		{"style.css", "\ue749"},       //  nf-dev-css3
		{"README.markdown", "\uf48a"}, //  nf-oct-markdown
		{"lib.rs", "\ue7a8"},          //  nf-dev-rust
		{"tsconfig.json", "\ue60b"},   //  nf-seti-json
		{"deploy.yml", "\ue6a8"},      //  nf-seti-yaml
		{"query.sql", "\ue706"},       //  nf-dev-database
		{"logo.png", "\uf1c5"},        //  nf-fa-file_image_o
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := For(tc.name, false, false); got != tc.want {
				t.Fatalf("For(%q) = %q (%U), want %q (%U)", tc.name, got, []rune(got), tc.want, []rune(tc.want))
			}
		})
	}
}
...
// this lookup tier exists at all. Expectations are literal codepoints
// for the same reason as TestForByExtension: reading them back out of
// nameIcons would assert nothing about the mapping.
func TestForByFullName(t *testing.T) {
	cases := []struct {
		...
		{"Makefile", "\uf425"},    //  nf-oct-tools
		{"GNUmakefile", "\uf425"}, //  nf-oct-tools
		{"go.mod", "\ue627"},      //  nf-seti-go
		{"go.sum", "\ue627"},      //  nf-seti-go
		{".gitignore", "\ue702"},  //  nf-dev-git
		{"LICENSE", "\uf02d"},     //  nf-fa-book (matched case-insensitively)
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := For(tc.name, false, false); got != tc.want {
...
// detection would lie either way. Detection is pinned to the OPPOSITE
// answer in each half so the assertion can only pass if the mode
// short-circuits before Detect() is consulted.
func TestResolveExplicitOverrides(t *testing.T) {
	...
	fcList = func(context.Context) ([]byte, error) {
		return []byte("DejaVu Sans\nLiberation Mono\n"), nil
	}
	detectTimeout = -time.Second // see TestResolveAutoFollowsDetection
	if !Resolve(userconfig.IconsOn) {
		t.Fatalf("IconsOn should resolve true even when detection says no")
	}

	fcList = func(context.Context) ([]byte, error) {
		return []byte("Hack Nerd Font Mono\n"), nil
	}
	detectTimeout = prevTimeout
	if Resolve(userconfig.IconsOff) {
		t.Fatalf("IconsOff should resolve false even when detection says yes")
	}
}

// TestResolveAutoFollowsDetection pins the "auto" contract in both
// directions: the mode must hand the decision to Detect() and return
// whatever it says. The detection answer is injected through the fcList
// seam rather than read off the host, so this holds identically on a
// dev box with Nerd Fonts installed and on a bare CI runner. The
// negative half also spends the shared deadline up front, which is what
...
func TestResolveAutoFollowsDetection(t *testing.T) {
	...
	if !Resolve(userconfig.IconsAuto) {
		t.Fatalf("auto must resolve true when detection finds a Nerd Font")
	}

	fcList = func(context.Context) ([]byte, error) {
		return []byte("DejaVu Sans\nLiberation Mono\n"), nil
	}
	detectTimeout = -time.Second // deadline already spent: skip the fs walk
	if Resolve(userconfig.IconsAuto) {
		t.Fatalf("auto must resolve false when detection finds nothing")
	}
}
...
// TestColorForKnownExtension verifies the extension lookup returns the
// language's brand colour — Go's gopher cyan, spelled out here rather
// than read back from extColors so a mis-keyed or recoloured entry is
// a failure instead of a self-fulfilling comparison.
func TestColorForKnownExtension(t *testing.T) {
	fb := tcell.NewRGBColor(1, 2, 3)
	want := tcell.NewRGBColor(0, 173, 216) // gopher cyan
	if got := ColorFor("main.go", false, fb); got != want {
		t.Fatalf("ColorFor(main.go) = %v, want gopher cyan %v", got, want)
	}
	want = tcell.NewRGBColor(204, 52, 45) // ruby red
	if got := ColorFor("app.rb", false, fb); got != want {
		t.Fatalf("ColorFor(app.rb) = %v, want ruby red %v", got, want)
	}
}
...
// Expectations are literal for the same reason as the extension test.
func TestColorForKnownName(t *testing.T) {
	fb := tcell.NewRGBColor(1, 2, 3)
	want := tcell.NewRGBColor(36, 150, 237) // docker blue
	if got := ColorFor("Dockerfile", false, fb); got != want {
		t.Fatalf("ColorFor(Dockerfile) = %v, want docker blue %v", got, want)
	}
	want = tcell.NewRGBColor(0, 173, 216) // go.mod keeps the gopher cyan
	if got := ColorFor("go.mod", false, fb); got != want {
		t.Fatalf("ColorFor(go.mod) = %v, want %v", got, want)
	}
}
